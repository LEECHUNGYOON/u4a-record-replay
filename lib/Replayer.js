        ...
        try {
            ...
            this.page.on('requestfailed', (request) => {
                ...
                if (failure) {
                    ...
                    this.consoleErrors.push(errorData);
                    this.emit('error', errorData); // 🆕 이벤트 발생
                }
            });
    ...
    async play(recordData) {
        ...
        for (let i = 0; i < actions.length; i++) {
            if (!this.isPlaying) {
                ...
                return { RETCD: 'E', STCOD: ReplayerStatusCode.REPLAY_STOPPED, MSGTX: '사용자에 의해 Replay가 중지되었습니다.' };
            }
            if (!this.browser || !this.page) {
                ...
                return { RETCD: 'E', STCOD: ReplayerStatusCode.BROWSER_CLOSED, MSGTX: '브라우저가 종료되었습니다.' };
            }
            ...
            try {
                ...
                if (error.code === ReplayerStatusCode.BUSY_TIMEOUT) {
                    return { RETCD: 'E', STCOD: ReplayerStatusCode.BUSY_TIMEOUT, MSGTX: error.message };
                }

                return { RETCD: 'E', STCOD: ReplayerStatusCode.ACTION_FAILED, MSGTX: `[Step ${i+1}] ${error.message}` };
            }
        }
        ...
        return { RETCD: 'S', DATA: { consoleErrors: this.consoleErrors } };
    }    
